...
useEffect(() => {
    async function fetchOverlords() {
        const fetchedOverlords = await getOverlords();
        if (fetchedOverlords.length > 0) {
        fetchedOverlords.sort((a, b) => {
            // First, sort by rarity
            if (b.rarity !== a.rarity) {
                return b.rarity - a.rarity; // Sort by rarity descending
            } else {
        ...
        setOverlords(fetchedOverlords);
    }
    else {
      console.log('No overlords')
    }
  }
    fetchOverlords();
}, []);
...
const toggleExpand = (index: number) => {
  ...
  if (expandedIndex === index) {
    ...
    setExpandedNftData(overlords[index]); // Expand the clicked item
    setSelectedNftContractId('');
  }
};
...
const defaultImageIndex = expandedIndex === null ? findHighestvotingpowerNftIndex(overlords) : null;
...
{overlords.length > 0 && (
  <div className={styles.collectiontwoColumnLayout}>
    <ul className={styles.collection} style= { { backgroundColor: '#280ecd9b' , color: '#FFFFFF' } } >
      <h1>The Overlords &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; The first among equals &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; Principes</h1>
      {overlords.map((nft, index) =>(
        <li key={index} className={`${styles.nft} ${expandedIndex === index ? styles.expanded : ''}`}  style= { { backgroundColor: '#280ecd9b'  } }  onClick={() => toggleExpand(index)}>
